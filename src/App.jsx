...
async function sendMagicLink(email) {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.msg || err.message || "Failed to send magic link");
  }
}
...
export default function App() {
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      setOtpSent(true);
      showMsg("✅ Magic link sent to " + email + " — check your inbox!");
    } catch (e) {
      showMsg("Failed to send magic link: " + e.message, true);
    }
    setLoading(false);
  };
      ...
      <div style={styles.card}>
        ...
        {!otpSent ? (
          ...
          <div style={{ textAlign: "center", padding: 20 }}>
            ...
            <p style={{ color: "#00b4d8", fontWeight: 700 }}>
              Magic link sent to {email}!
            </p>
            <p style={{ color: "#6b8399", fontSize: 13 }}>
              Check your email and click the link to login
            </p>
            <button style={styles.btnOutline} onClick={resetEmail}>
              Change Email
            </button>
          </div>
        )}
