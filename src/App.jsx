...
export default function App() {
      ...
      if (token) {
        ...
        fetch(`${SUPABASE_URL}/auth/v1/user`, {
          ...
          .then(async user => {
            ...
            if (rows?.length > 0) { persistStudent(rows[0], token); setScreen("home"); }
      ...
      try {
        ...
        if (id && token) {
          ...
          supabaseCall("students", "GET", null, `?id=eq.${id}`, token)
            .then(rows => {
              if (rows?.[0]) { setStudent(rows[0]); setScreen("home"); }
  ...
  const registerStudent = async () => {
    const { full_name, college, year_of_study, roll_number, phone } = regData;
    if (isBlank(full_name) || isBlank(college) || isBlank(year_of_study) || isBlank(roll_number) || isBlank(phone))
  ...
  const openRazorpay = async () => {
    ...
    // FIX (Razorpay script can load multiple times): idempotent module-level guard
    ...
    const options = {
      ...
      handler: async function (response) {
        // FIX (Payment not verified securely): Send all three Razorpay fields to
        // /api/verify-payment (Vercel serverless) for HMAC signature verification.
        // The server activates the subscription atomically — never the frontend.
        ...
        try {
          ...
          // Re-fetch DB state — server has already written the subscription
          const fresh = await supabaseCall("students", "GET", null, `?id=eq.${student.id}`, accessToken);
          if (fresh?.[0]) persistStudent(fresh[0], accessToken);
      ...
      <div style={styles.app}>
        ...
        <div style={{ padding: "16px 16px 100px" }}>
          ...
          <div style={styles.card}>
            ...
            <label style={{ fontSize: 12, color: "#6b8399" }}>Phone Number *</label>
            <input style={styles.input} placeholder="10-digit mobile number" type="tel"
              inputMode="numeric" maxLength={10} value={regData.phone}
