...
const firebaseConfig = {
  apiKey: "AIzaSyCRKr-bsDACbsreTpKw-TVaTgQVS9zCTK0",
  authDomain: "medcase-tn.firebaseapp.com",
  projectId: "medcase-tn",
  storageBucket: "medcase-tn.firebasestorage.app",
  messagingSenderId: "923467581138",
  appId: "1:923467581138:web:a91308249ebae698e60f0b",
};
...
export default function App() {
  ...
  // ─── RECAPTCHA FIX ───────────────────────────────────────────────────
  // Always clear + wipe innerHTML before creating a new verifier.
  ...
  const clearRecaptcha = () => {
    ...
    // Replace the container with a fresh element to guarantee no prior render state
    const old = document.getElementById("recaptcha-container");
    if (old && old.parentNode) {
      const fresh = document.createElement("div");
      fresh.id = "recaptcha-container";
      fresh.style.cssText = "position:absolute;top:0;left:0;";
      old.parentNode.replaceChild(fresh, old);
    }
  };

  const getRecaptchaVerifier = () => {
    clearRecaptcha(); // always destroy old one and swap in a fresh node first
  ...
  const verifyOTPAndLogin = async () => {
    ...
    if (!confirmationResult) return showMsg("Please request OTP first", true);
    setLoading(true);
    try {
  ...
  const registerStudent = async () => {
    ...
    try {
      const newStudent = await supabase("students", "POST", {
        full_name, phone, college, year_of_study, roll_number,
  ...
  const openRazorpay = () => {
    ...
    const options = {
      ...
      prefill: { contact: `+91${student?.phone}` },
      theme: { color: "#00b4d8" },
  ...
  if (screen === "login") return (
    <div style={styles.app}>
      {/* Must stay in DOM; cleared via innerHTML before each new RecaptchaVerifier */}
      <div id="recaptcha-container" style={{ position: "absolute", top: 0, left: 0 }} />
        ...
        {!otpSent ? (
          ...
          <>
            <div style={{ marginTop: 12 }}>
              <label style={{ fontSize: 13, color: "#6b8399" }}>Enter OTP sent to +91{phone}</label>
              <input
            ...
            <button style={styles.btnOutline} onClick={resetPhone}>Change Number</button>
          </>
        )}
      </div>
    ...
    <div style={styles.app}>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ ...styles.card, textAlign: "center", border: "1px solid rgba(0,180,216,0.4)" }}>
          ...
          <div style={{ fontSize: 36, fontWeight: 800, color: "#e8f4f8" }}>
            ₹30<span style={{ fontSize: 16, color: "#6b8399" }}>/month</span>
          </div>
          <p style={{ color: "#6b8399", fontSize: 13, margin: "10px 0 16px" }}>
            Unlimited access to all cases across 38 TN colleges
          </p>
          {["Unlimited case viewing", "All 38 TN Medical Colleges", "Filter by ward & college", "Upload & earn credits", "Priority new cases"].map(f => (
            <div key={f} style={{ textAlign: "left", padding: "6px 0", fontSize: 14, color: "#e8f4f8" }}>✅ {f}</div>
          ))}
          <button style={{ ...styles.btn, marginTop: 16 }} onClick={openRazorpay} disabled={loading}>
            {loading ? "Processing..." : "Pay ₹30 with Razorpay 💳"}
          </button>
          <p style={{ fontSize: 11, color: "#6b8399", margin: 0 }}>Secure payment · UPI, Cards, NetBanking accepted</p>
        </div>
